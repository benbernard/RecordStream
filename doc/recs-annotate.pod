...
=head1 recs-annotate --help-all
 ...
 Usage: recs-annotate <args> <expr> [<files>]
    <expr> is evaluated as perl on each record of input (or records from <files>) with $r set to a App::RecordStream::Record object and $line set to the current line number (starting at 1). Records are analyzed for changes, those changes
    are applied to each successive record that matches --keys
 
    Only use this script if you have --keys fields that are repeated, otherwise recs-xform will be faster
 
 IMPORTANT SNIPPET NOTE
    Because of the way annotations are recorded, you cannot use UNSHIFT or SPLICE on array refs that already exist in the record you are modifiying. Additinally, deletes, removes, unshifts, and other 'removing' operations will not apply
    to later records. If you need this behavior, consider using recs-xform
