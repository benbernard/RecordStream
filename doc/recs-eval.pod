...
=head1 recs-eval --help-all
 ...
 Usage: recs-eval <args> <expr> [<files>]
    ...
    and $line set to the current line number (starting at 1). The result of each evaluation is printed on a line by itself (this is
    not a recs stream). See App::RecordStream::Record for help on what the $r object can do. See --help-snippets for more
    information on code snippets
