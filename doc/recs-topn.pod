...
=head1 recs-topn --help-all
 ...
 Usage: recs-topn <args> [<files>]
    Outputs the top n records from input stream or from <files>. You may segment the input stream based on a list of keys such that unique values of keys are treated as distinct input streams. This enables top n listings per value
    ...
    --delimiter <string>         String used internally to delimit values when performing a topn on a keyspec that inlcudeds multiple keys. This value defaults to "9t%7Oz%]" which may - under unusual and bizarre corner cases - cause
                                 false positive key matches if your values contain this value. You can set this to any string.
