...
`recs xform` is both simple and powerful: it executes arbitrary, inline JavaScript on
each record. The fields can be accessed as `{{field}}` expressions using the
keyspec syntax. In this case we are using a replace call to strip the key off of
`get.cgi` requests. At this point our data is ready to be aggregated and made
into statistics.
...
Aggregators are one of the most powerful features of recs. Some of the most
powerful are:
...
Now that the collation has been done the records have the numbers we desire, but
they are neither in a useful order, nor a pretty format.
...
5 top offenders. And we use `recs totable` to convert those top five to a nicely
...
Should you find yourself locked in mortal combat with an unspeakable horror of
your own you can always turn to `--help`. All recs scripts come equipped with
detailed usage instructions triggered by the `--help` option.

## See Also

- See [Getting Started](/guide/getting-started) for an overview of the system
- See [Examples](/guide/examples) for a set of simple recs examples
- See [Cookbook](/guide/cookbook) for real-world recipes
