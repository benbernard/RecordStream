...
export class CorrelationAggregator implements Aggregator<Ord2BivState | null> {
  fieldX: string;
  fieldY: string;
  ...
  squish(state: Ord2BivState | null): JsonValue {
    ...
    return (sumxy * sum1 - sumx * sumy) /
      Math.sqrt((sumx2 * sum1 - sumx ** 2) * (sumy2 * sum1 - sumy ** 2));
  }
}

aggregatorRegistry.register("corr", {
  create: (fieldX: string, fieldY: string) => new CorrelationAggregator(fieldX, fieldY),
  argCounts: [2],
  shortUsage: "find correlation of provided fields",
  longUsage: "Usage: corr,<field1>,<field2>\n   Correlation of specified fields.",
