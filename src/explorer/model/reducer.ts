...
export function pipelineReducer(
  ...
  switch (action.type) {
    ...
    case "UPDATE_STAGE_ARGS": {
      ...
      // This avoids a race where dismissing EditStageModal with unchanged args
      // invalidates a freshly-computed cache result.
      const oldArgs = stage.config.args;
      const newArgs = action.args;
      if (
        oldArgs.length === newArgs.length &&
        oldArgs.every((a, i) => a === newArgs[i])
      ) {
        return state;
      }
      ...
      stages.set(action.stageId, {
        ...stage,
        config: { ...stage.config, args: [...newArgs] },
      });
    ...
    // ── Column highlight ────────────────────────────────────────
    case "MOVE_COLUMN_HIGHLIGHT": {
      const current = state.inspector.highlightedColumn;
      const max = action.fieldCount - 1;
      if (max < 0) return state;

      let next: number;
      if (current === null) {
        next = action.direction === "left" ? max : 0;
      } else if (action.direction === "left") {
        next = current <= 0 ? 0 : current - 1;
      } else {
        next = current >= max ? max : current + 1;
      }
      return {
        ...state,
        inspector: { ...state.inspector, highlightedColumn: next },
      };
    }

    case "CLEAR_COLUMN_HIGHLIGHT":
      return {
        ...state,
        inspector: { ...state.inspector, highlightedColumn: null },
      };
...
function wouldBeNoop(state: PipelineState, action: PipelineAction): boolean {
  switch (action.type) {
    ...
    case "UPDATE_STAGE_ARGS": {
      ...
      // No-op if args haven't actually changed
      const oldArgs = stage.config.args;
      const newArgs = action.args;
      return (
        oldArgs.length === newArgs.length &&
