/**
 * Integration tests for the TUI pipeline flow.
 *
...
import {
  createInitialState,
  pipelineReducer,
} from "../../../src/tui/model/reducer.ts";
import { executeToStage } from "../../../src/tui/executor/executor.ts";
import {
  getActivePath,
  getEnabledStages,
  isDownstreamOfError,
} from "../../../src/tui/model/selectors.ts";
import {
  exportAsPipeScript,
...
describe("Input op integration", () => {
  test("fromcsv as first stage with file input", async () => {
    ...
    const tmpFile = `/tmp/recs-tui-integration-${Date.now()}.csv`;
